<?php
// This file is part of Moodle - http://moodle.org/
//
// Moodle is free software: you can redistribute it and/or modify
// it under the terms of the GNU General Public License as published by
// the Free Software Foundation, either version 3 of the License, or
// (at your option) any later version.
//
// Moodle is distributed in the hope that it will be useful,
// but WITHOUT ANY WARRANTY; without even the implied warranty of
// MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
// GNU General Public License for more details.
//
// You should have received a copy of the GNU General Public License
// along with Moodle.  If not, see <http://www.gnu.org/licenses/>.

/**
 * English language strings for report_courseradar.
 *
 * @package    report_courseradar
 * @copyright  2025 Sergio Comerón <sergiocomeron@icloud.com>
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */

defined('MOODLE_INTERNAL') || die();

$string['aboveaverage']        = 'Above average';
$string['activityovertime']    = 'Activity over time';
$string['activityovertime_desc'] = 'Daily evolution of student interactions in the selected period.';
$string['activitypattern']     = 'Activity pattern (day x hour)';
$string['activitypattern_desc'] = 'Interactions grouped by day of week and time slot — reveals when students are most active.';
$string['adjustperiod']        = 'Adjust period';
$string['allstudentsviewed']   = 'All students have viewed this!';
$string['allvisited']          = "You've opened every available resource. Great work!";
$string['analyzingperiod']     = 'Analysing period: {$a->from} – {$a->to}';
$string['applyfilter']         = 'Apply filter';
$string['atrisk']              = 'Low activity students';
$string['atrisk_info']         = 'Students with no interactions or very low activity in the selected period.';
$string['atrisk_lowactivity']  = 'Low engagement (< 30% resources visited)';
$string['atrisk_noactivity']   = 'No interactions';
$string['avgdedication']       = 'Avg. time spent';
$string['avgdedication_desc']  = 'Per student with recorded sessions';
$string['avgengagement']       = 'Avg. engagement';
$string['avgviews'] = 'Avg. views / module';
$string['baseurl']             = 'API base URLs';
$string['baseurl_desc']        = 'One URL per line. Used only if Zoom UDIMA credentials are not reused.';
$string['belowaverage']        = 'Below average';
$string['campus']              = 'Campus';
$string['campus_desc']         = 'Sent as Origen. UDIMA uses the Moodle username; CEF uses idnumber.';
$string['chartdateformat']     = '%d %b';
$string['classaverage']        = 'Class average';
$string['clientid']            = 'OAuth client ID';
$string['clientid_desc']       = 'Optional if Zoom UDIMA credentials are reused.';
$string['clientsecret']        = 'OAuth client secret';
$string['clientsecret_desc']   = 'Optional if Zoom UDIMA credentials are reused.';
$string['compareclass']        = 'How you compare with the class';
$string['compareclass_desc']   = 'Your figures next to the (anonymous) class average. Other students are never identified.';
$string['completednoftrack']   = 'No completion tracking';
$string['completion']          = 'Completion';
$string['completion_desc']     = 'Students who completed it';
$string['completiondisabled']  = 'Activity completion is not enabled for this course.';
$string['completionstu']       = 'Completion';
$string['completionstu_desc']  = 'Completed / tracked';
$string['conexiones']          = 'Live / on-demand sessions';
$string['conexiones_desc']     = 'Time in live Zoom sessions and Vimeo recording views (UDIMA API).';
$string['conexionesallupdated'] = 'All students updated: {$a}';
$string['conexionesapiheading'] = 'Connections API (Zoom / Vimeo)';
$string['conexionesapiheading_desc'] = 'POST /Informes/GetInformeConexionesAlumno. By default the OAuth settings of the Zoom UDIMA block are reused.';
$string['conexionescoldate']   = 'Date';
$string['conexionescolduration'] = 'Duration';
$string['conexionescoltime']   = 'Time';
$string['conexionesdelayed']   = 'On-demand sessions';
$string['conexionesdelayed_desc'] = 'Time spent watching recordings';
$string['conexionesdetail']    = 'Session details';
$string['conexioneserror']     = 'Could not load connection data';
$string['conexionesexport']    = 'Export sessions';
$string['conexionesexporting'] = 'Refreshing all students, then download…';
$string['conexioneslive']      = 'Live sessions';
$string['conexioneslive_desc'] = 'Time connected to live sessions';
$string['conexionesnodetail']  = 'No session details';
$string['conexionesnone']      = 'No connection data';
$string['conexionesprogress']  = 'Updating {$a->done} / {$a->total} students';
$string['conexionesreusezoom'] = 'Reuse Zoom UDIMA API credentials';
$string['conexionesreusezoom_desc'] = 'Use the OAuth token, base URLs and campus already configured in block_zoom_udima.';
$string['conexionesstale']     = 'Updated {$a}';
$string['conexionesunavailable'] = 'Connections API is not configured';
$string['conexionesupdated']   = 'Updated {$a}';
$string['conexionesupdating']  = 'Updating…';
$string['courseradar:view']    = 'View Course Radar report';
$string['coursevisits']        = 'Course visits';
$string['coursevisits_desc']   = 'Times students accessed the course home';
$string['coverage']            = 'Coverage';
$string['coverage_desc']       = '% enrolled students who viewed it';
$string['datefrom']            = 'From';
$string['dateto']              = 'To';
$string['daysinactive']    = 'Days inactive';
$string['daysinactive_desc']   = 'Days since last interaction';
$string['dedication']          = 'Time spent';
$string['dedication_desc']     = 'Time connected to the course';
$string['details']             = 'Detail';
$string['engdistribution']     = 'Engagement distribution';
$string['engdistribution_desc'] = 'Number of students in each resource-coverage quartile (based on % of resources visited).';
$string['filterbytype']        = 'Filter by type:';
$string['haventviewed']        = "Haven't viewed";
$string['haveviewed']          = 'Have viewed';
$string['hidden']              = 'Hidden';
$string['lastaccess']          = 'Last access';
$string['lastaccess_desc']     = 'Date of last recorded access';
$string['lastactivity']        = 'Last activity';
$string['lastactivity_desc']   = 'Most recent resource interaction';
$string['lastcoursevisit']     = 'Last course visit';
$string['lastcoursevisit_desc'] = 'Last time student accessed the course home';
$string['loadingdata']         = 'Loading';
$string['modules'] = 'Modules';
$string['moduletypesummary'] = 'Interactions by resource type';
$string['moduletypesummary_desc'] = 'Total views per resource type, normalised to the most-viewed type.';
$string['mostviewed']          = 'Most viewed';
$string['msgsent']             = 'Message sent';
$string['myactivity_desc']     = 'Your daily interactions with course resources.';
$string['neveraccessed']   = 'Never';
$string['noactivitydata']      = 'No activity data for this period.';
$string['nointeractions']      = 'No interactions recorded in this period.';
$string['none']                = 'None';
$string['nostudents']          = 'No students enrolled in this course.';
$string['notifyrisk']          = 'Message low activity students';
$string['notifyrisk_placeholder'] = 'Write your message here...';
$string['notviewed']           = 'not seen';
$string['noviewsyet']          = 'No views yet.';
$string['pendingresources']    = "Resources you haven't opened yet";
$string['pendingresources_desc'] = 'Open these to keep up with the course.';
$string['plugindesc']          = 'Track student interactions with course resources and activities.';
$string['pluginname']          = 'Course Radar';

$string['resetfilter']         = 'Reset';
$string['resetsort']           = 'Back to section view';
$string['resource']            = 'Resource / Activity';
$string['resourceactivity']    = 'Resources & Activities';
$string['resourceactivity_desc'] = 'Coverage and interactions per resource, grouped by course section. Click a column header to sort.';
$string['resourcesvisited']    = 'Resources visited';
$string['resourcesvisited_desc'] = 'Different resources opened';
$string['riskscore']           = 'Score';
$string['riskscore_desc']      = 'Engagement score (0–100)';
$string['scatter_desc']        = 'Each dot is a student. X axis: % resources visited. Y axis: engagement score. Click a dot to open the student profile.';
$string['scatter_title']       = 'Student comparison: resources vs. engagement';
$string['scatter_xaxis']       = '% Resources visited';
$string['scatter_yaxis']       = 'Engagement score';
$string['scope']               = 'OAuth scope';
$string['scope_desc']          = 'Optional if Zoom UDIMA credentials are reused.';
$string['scoredist_desc']       = 'Each bar shows how many students fall in that score band. The score (0–100) combines three factors: % of resources visited, days since last access, and activity completion (if enabled). A student who visits few resources but logged in recently scores higher than one who visited the same resources weeks ago.';
$string['scoredist_title']      = 'Engagement score distribution';
$string['scorehelp_completion']    = '% of tracked activities completed (only counted when completion tracking is enabled).';
$string['scorehelp_factors']       = 'The engagement score (0–100) is a weighted combination of these factors for each student:';
$string['scorehelp_formula']       = 'Applied formula';
$string['scorehelp_formula_basic'] = 'score = 0.50 × resources + 0.50 × recency';
$string['scorehelp_formula_full']  = 'score = 0.35 × resources + 0.35 × recency + 0.30 × completion';
$string['scorehelp_recency']       = 'Recency of last access: 100 if active today, decreasing linearly to 0 at 30 days of inactivity (0 if the student has never interacted).';
$string['scorehelp_resources']     = '% of distinct course resources the student has visited.';
$string['scorehelp_title']         = 'How is the engagement score calculated?';
$string['searchstudent']       = 'Search student...';
$string['sendmsg']             = 'Send message';
$string['showhidden']          = 'Show hidden activities';
$string['sortby']              = 'Click to sort';
$string['student']             = 'Student';
$string['studentcoverage_desc'] = '% of course resources visited';
$string['studentengagement']   = 'Student Engagement';
$string['studentengagement_desc'] = 'Individual activity per student. Click any column header to sort. Expand a row to see resource-level detail.';
$string['studentintro']        = 'This is your personal activity summary for this course. Only you and your teachers can see it.';
$string['studentshowchart']    = 'Activity over time';
$string['studentshowchart_desc'] = 'Show the student\'s daily interactions chart.';
$string['studentshowcomparison'] = 'Comparison with the class';
$string['studentshowcomparison_desc'] = 'Show "you vs class average" bars for the metrics that are enabled. The average is anonymous.';
$string['studentshowcompletion'] = 'Completion';
$string['studentshowcompletion_desc'] = 'Show the percentage of tracked activities completed (only if the course has completion tracking).';
$string['studentshowconexiones'] = 'Live / on-demand sessions';
$string['studentshowconexiones_desc'] = 'Show live and on-demand session time.';
$string['studentshowcoverage'] = 'Resources visited';
$string['studentshowcoverage_desc'] = 'Show the percentage of course resources the student has opened.';
$string['studentshowdaysinactive'] = 'Days inactive';
$string['studentshowdaysinactive_desc'] = 'Show days since the student\'s last interaction.';
$string['studentshowdedication'] = 'Time spent';
$string['studentshowdedication_desc'] = 'Show time spent in the course.';
$string['studentshowpending']  = 'Pending resources';
$string['studentshowpending_desc'] = 'Show the list of resources the student has not opened yet.';
$string['studentshowscore']    = 'Score';
$string['studentshowscore_desc'] = 'Show the engagement score (0–100).';
$string['studentviewheading']  = 'Student view';
$string['studentviewheading_desc'] = 'Choose what an enrolled student sees in their personal Course Radar summary. The teacher report is not affected.';
$string['studentviews_desc']   = 'Total accesses to all resources';
$string['tab_overview']        = 'Overview';
$string['tab_resources']       = 'Resources';
$string['tab_students']        = 'Students';
$string['taskrefreshconexiones'] = 'Refresh Zoom/Vimeo connection totals';
$string['times']               = 'views';
$string['tokenurl']            = 'OAuth token URL';
$string['tokenurl_desc']       = 'Optional if Zoom UDIMA credentials are reused.';
$string['topunseen']       = 'Least visited resources';
$string['topunseeninfo']   = 'Visible resources with lowest student coverage (100% excluded). Top 10 shown per active filter.';
$string['totalinteractions']   = 'Total interactions';
$string['totalresources']      = 'Total resources';

$string['totalviews']          = 'Total views';
$string['totalviews_desc']     = 'Total accesses by all students';
$string['type']                = 'Type';
$string['uniquestudents']      = 'Students';
$string['uniquestudents_desc'] = 'Unique students / enrolled';

$string['viewprofile']         = 'View profile';
$string['weeklyactivity'] = 'Weekly activity';
$string['weeklyaggregated']    = 'Aggregated by week (period > 90 days)';
$string['weekvspreview']       = 'vs. last week';
$string['you']                 = 'You';
